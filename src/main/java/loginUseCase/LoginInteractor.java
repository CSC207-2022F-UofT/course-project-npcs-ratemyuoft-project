...
public class LoginInteractor implements LoginInputBoundary {
    ...
    /**
     * @param loginOutputBoundary is needed to pass results of performing some method
     * @param loginDataAccess is needed to access a DataBase
     ...
     *  DataBase failed.
     */
    public LoginInteractor(LoginOutputBoundary loginOutputBoundary, LoginDataAccess loginDataAccess) throws ClassNotFoundException{
        this.loginOutputBoundary = loginOutputBoundary;
        this.loginDataAccess = loginDataAccess;
        try{
            users = loginDataAccess.importUsers();
        }catch (IOException e){
            users = new UserList();
    ...
    public void userLogin(String username, String password) throws InvalidInputException, IOException, ClassNotFoundException {
        ...
        if(checkInput(username)&& checkInput(password) && checkIfUserExists(username)&& !checkUserStatus(username) &&
            ...
            try{
                ...
                loginDataAccess.saveUser(users);
